...
************************************************************************************
* Types of farming households: three KINDS
************************************************************************************

SORT CASES  BY HH_ID.

************************************************************************************
* Farming HH where Head is skilled agricultural and not an employee

COMPUTE HH_F_HoH=((600 <= p23_occupation and p23_occupation  <700 ) and (p03_relationship = 1) and 
(p22_activity_status=5 or  p22_activity_status=3)).
 exe.
...
************************************************************************************
* Farming HH where head or spouse are skilled agricultural not an employee

COMPUTE HH_F_HoH_Sp =((600 <= p23_occupation and p23_occupation  <700 ) and (p03_relationship = 1 or p03_relationship = 2)
and(p22_activity_status=5 or p22_activity_status=3)).
 exe.
...
************************************************************************************
* Farming HH where any relative is skilled agricultural not an employee

COMPUTE HH_F_Any =((600 <= p23_occupation and p23_occupation  <700 ) and 
(p03_relationship <= 8) and(p22_activity_status=5 or p22_activity_status=4 or  p22_activity_status=3)).
exe.
...
IF  (FarmingHH_F_HoH = 0 or missing (FarmingHH_F_HoH) ) HH_Type_HoH = 0. 
...
IF  (FarmingHH_F_HoH_Sp = 0 or missing (FarmingHH_F_HoH_Sp)) HH_Type_HoH_Sp=0. 
IF  (FarmingHH_F_HoH_Sp = 1 & Over60_HoH = 1 ) HH_Type_HoH_Sp=1.
IF  (FarmingHH_F_HoH_Sp = 1 & Over60_HoH = 0 & Parents_coresHH = 1) HH_Type_HoH_Sp=2.
IF  (FarmingHH_F_HoH_Sp = 1 & Over60_HoH = 0 & Parents_coresHH = 0) HH_Type_HoH_Sp=3.
...
IF  (FarmingHH_F_Any = 0 or missing(FarmingHH_F_Any )) HH_Type_Any=0. 
...
VALUE LABELS HH_Type_Any 0  'not farming' 1 'HoH over 60' 2 'HoH under 60 AND parents cores' 3 'HoH under 60 and parents not cores'.
...
IF  (p04_sex = 1 AND p03_relationship NE 1 & 
    ((p22_activity_status = 5 or p22_activity_status = 9) or 
((p23_occupation > 600 and p23_occupation  <700 ) or p23_occupation =921))) Avail_Labour_M = 1. 
exe.
...
IF  (p04_sex = 2 AND p03_relationship NE 1 & 
    ((p22_activity_status = 5 or p22_activity_status = 9) or 
((p23_occupation > 600 and p23_occupation  <700 ) or p23_occupation =921))) Avail_Labour_F = 1. 
exe.
...
IF  (h39g_car_truck_van + h39h_motorcycle_moped + h39j_4_wheel_tractor= 6) No_Vehicle = 1. 
exe.
...
CROSSTABS
  /TABLES= p09_disablity_dummy_sum BY p05_age_binned BY p04_sex BY  HH_Type_HoH BY batch_state_region 
...
CROSSTABS
  /TABLES= p09_disablity_dummy_sum BY p05_age_binned BY p04_sex BY  HH_Type_HoH_Sp BY batch_state_region  
  /FORMAT=AVALUE TABLES
  /CELLS=COUNT
  /COUNT ROUND CELL.
...
CROSSTABS
  /TABLES=  p09_disablity_dummy_sum BY p05_age_binned BY p04_sex BY  HH_Type_Any BY batch_state_region  
  /FORMAT=AVALUE TABLES
  /CELLS=COUNT
  /COUNT ROUND CELL.
...
CROSSTABS
  /TABLES=  HH_N_Avail_Labour HH_N_Avail_Labour_M HH_N_Avail_Labour_F  BY  HH_Type_HoH BY p04_sex BY batch_state_region
  /FORMAT=AVALUE TABLES
  /CELLS=COUNT
  /COUNT ROUND CELL.
...
CROSSTABS
  /TABLES= HH_N_Avail_Labour HH_N_Avail_Labour_M HH_N_Avail_Labour_F  BY  HH_Type_HoH_Sp BY p04_sex BY batch_state_region
  /FORMAT=AVALUE TABLES
  /CELLS=COUNT
  /COUNT ROUND CELL.
...
CROSSTABS
  /TABLES= HH_N_Avail_Labour HH_N_Avail_Labour_M HH_N_Avail_Labour_F  BY   HH_Type_Any BY p04_sex BY batch_state_region
  /FORMAT=AVALUE TABLES
  /CELLS=COUNT
  /COUNT ROUND CELL.
...
CROSSTABS
  /TABLES= HH_Type_HoH BY HH_No_Vehicle BY p04_sex BY  batch_state_region 
  /FORMAT=AVALUE TABLES
  /CELLS=COUNT
  /COUNT ROUND CELL.
...
CROSSTABS
  /TABLES=  HH_Type_HoH_Sp BY HH_No_Vehicle BY p04_sex BY  batch_state_region 
  /FORMAT=AVALUE TABLES
  /CELLS=COUNT
  /COUNT ROUND CELL.
...
CROSSTABS
  /TABLES= HH_Type_Any BY HH_No_Vehicle BY p04_sex BY  batch_state_region 
  /FORMAT=AVALUE TABLES
  /CELLS=COUNT
  /COUNT ROUND CELL.
