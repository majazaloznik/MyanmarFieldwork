...
2. Import into R and produce codebook, varlist:

* Run `\survey\scripts\01-Import.R`, which imports `\survey\data\Myanmar Ageing Farmer draft data set.sav` 
* cleans up
* Exports `\survey\data\survey.data.csv` simple .csv table of 700 vars
* Exports `\survey\data\survey.dataframe.RData` same as csv but with factor labels
* Exports `\survey\data\survey.dataset.RData` same as csv but with factor labels and var descriptions i.e. all the data that was in the spss `.sav` file.

3. Additional new variables are added in `\survey\scripts\01-Import.R` under section 6.  Then rerun the rest of the file to update teh saved export datafiles. 
